...
public class Board
{
    ...
    internal void FillWithDivisibleBy3()
    {
        ...
        foreach (NormalItem.eNormalType type in enumValues)
        {
        ...
        int extraSetsOf3 = remainingCells / 3;
        for (int i = 0; i < extraSetsOf3; i++)
        {
            NormalItem.eNormalType randomType = (NormalItem.eNormalType)enumValues.GetValue(UnityEngine.Random.Range(0, typeCount));
            for (int j = 0; j < 3; j++)
            {
        ...
        itemTypes = itemTypes.OrderBy(x => UnityEngine.Random.value).ToList();
